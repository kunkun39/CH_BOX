...
public class SearchPageDefault extends Fragment {
			...
			view.getViewTreeObserver().addOnGlobalLayoutListener(
					new OnGlobalLayoutListener() {
						...
						public void onGlobalLayout() {
							if (SearchPageDefault.this.activity != null
									&& !SearchPageDefault.this.view
											.isLayoutRequested()) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (view == null) {
			view = inflater
					.inflate(R.layout.search_page_idle, container, false);
		} else {
		...
		contentView = (LinearLayout) view
				.findViewById(R.id.search_idle_content);
		values.clear();
	...
	public boolean saveSentences(Activity activity, String text) {
		...
		try {
			...
			Cursor cursor = sqLiteDatabase.rawQuery(sqlString,
					new String[] { text });
	...
	public void deleteSentences(Activity activity, String sentence) {
		...
		database.getReadableDatabase().execSQL(sqlString,
				new Object[] { sentence });
	}
	...
	public void updateDatabase(Activity activity) {
		String sqlString = "DELETE FROM "
				+ DatabaseContainer.TABLE_NAME_SEARCH_HEAT
				+ " WHERE search_time BETWEEN 'NOW' AND DATE('-30 DAY')";
		if (database == null) {
			database = new DatabaseContainer(activity);
		}
	...
	private boolean addItemIntoLayout(View v) {
		...
		if (layouts.size() == 0) {
			...
			if (getViewRect(v).width() + testWidth > view.getWidth()) {
				android.view.ViewGroup.LayoutParams params = v
						.getLayoutParams();
	...
	private View generalItem(Context context, String text) {
		...
		view.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				DialogUtil.showAlertDialog(activity, "确定要删除?",
						String.valueOf(view.getText()),
						new DialogBtnOnClickListener() {
							@Override
							public void onSubmit(DialogMessage dialogMessage) {
								deleteSentences(activity,
										String.valueOf(view.getText()));
								addViews(SearchPageDefault.this.contentView);
							}
