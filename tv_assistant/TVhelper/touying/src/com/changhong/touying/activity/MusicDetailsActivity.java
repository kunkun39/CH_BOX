...
public class MusicDetailsActivity extends Activity {
	...
	private void initialViews() {
		...
		musicTotalTime = DateUtils
				.getTimeShow(selectedMusic.getDuration() / 1000);
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 0) {
					...
					String[] content = StringUtils.delimitedListToStringArray(
							((String) msg.obj), "|");
					String key = selectedMusic.getTitle() + "-"
				...
				if (msg.what == 1) {
					...
					Animation animation = AnimationUtils
							.loadAnimation(MusicDetailsActivity.this,
									R.anim.music_seekbar_out);
					seekbarLayout.startAnimation(animation);
	...
	private void initialEvents() {
		...
		seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
			...
			public void onStopTrackingTouch(SeekBar seekBar) {
				ClientSendCommandService.msg = "music:seekto:"
						+ seekBar.getProgress();
		...
		playImage.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				touYing(selectedMusic.getPath(), selectedMusic.getTitle(),
						selectedMusic.getArtist());
			}
		});
		...
		controlButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (isPausing) {
					...
					controlButton.setBackground(getResources().getDrawable(
			...
			if (NetworkUtils.isWifiConnected(MusicDetailsActivity.this)) {
				if (ClientSendCommandService.serverIpList.isEmpty()) {
					Toast.makeText(MusicDetailsActivity.this,
							"手机未连接电视，请确认后再投影", Toast.LENGTH_SHORT).show();
					return;
				}
				...
				if (seekbarLayout.getVisibility() == View.INVISIBLE) {
					seekbarLayout.setVisibility(View.VISIBLE);
					Animation animation = AnimationUtils.loadAnimation(
							MusicDetailsActivity.this, R.anim.music_seekbar_in);
					seekbarLayout.startAnimation(animation);
				}
				...
				if (musicPath
						.startsWith(NanoHTTPDService.defaultHttpServerPath)) {
					musicSelectedPath = musicPath.replace(
							NanoHTTPDService.defaultHttpServerPath, "")
							.replace(" ", "%20");
				} else {
				...
				String httpAddress = "http://" + ipAddress + ":"
						+ NanoHTTPDService.HTTP_PORT;
				String musicHttpAddress = httpAddress
						+ musicSelectedPath.replace(" ", "%20");
				...
				try {
					...
					if (lrc != null) {
						...
						if (lrcPath
								.startsWith(NanoHTTPDService.defaultHttpServerPath)) {
							lrcHttpAddress = lrcPath.replace(
									NanoHTTPDService.defaultHttpServerPath, "")
									.replace(" ", "%20");
						} else {
							for (String otherHttpServerPath : NanoHTTPDService.otherHttpServerPaths) {
								if (lrcPath.startsWith(otherHttpServerPath)) {
									lrcHttpAddress = musicPath.replace(
				...
				if (NanoHTTPDService.httpServer == null) {
					Intent http = new Intent(MusicDetailsActivity.this,
							NanoHTTPDService.class);
					startService(http);
				...
				MobilePerformanceUtils
						.openPerformance(MusicDetailsActivity.this);
			} else {
				Toast.makeText(MusicDetailsActivity.this, "请链接无线网络",
						Toast.LENGTH_SHORT).show();
			}
		} catch (Exception e) {
			e.printStackTrace();
			Toast.makeText(MusicDetailsActivity.this, "歌曲获取出错",
					Toast.LENGTH_SHORT).show();
		}
	}
}