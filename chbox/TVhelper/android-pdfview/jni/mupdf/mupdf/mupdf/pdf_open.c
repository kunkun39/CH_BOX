...
readoldtrailer(pdf_xref *xref, char *buf, int cap)
{
	...
	fz_throw("cannot read trailer : c:%s", xref->file->buffer->rp);
	t = fz_tell(xref->file);
		fz_throw("cannot read trailer : t:%d", t);
	...
	while (1)
	{
		...
		len = atoi(strsep(&s, " "));
		fz_throw("cannot read trailer : len:%d", len);
...
readoldxref(fz_obj **trailerp, pdf_xref *xref, char *buf, int cap)
{
	...
	while (1)
	{
		...
		if (s && *s != '\0')
		{
			fz_throw("broken xref section. proceeding anyway.");
			error = fz_seek(xref->file, -(2 + strlen(s)), 1);
			if (error)
...
readnewxref(fz_obj **trailerp, pdf_xref *xref, char *buf, int cap)
{
	...
	if (oid < 0 || oid >= xref->len)
	{
		if (oid == xref->len && oid < xref->cap)
		{
			...
			fz_throw("object id (%d %d R) out of range (0..%d)", oid, gen, xref->len - 1);
			xref->len ++;
		}
		else
		{
...
pdf_loadxref(pdf_xref *xref, char *filename)
{
	...
	fz_throw("readstartxref error:%d",1);
	...
	fz_throw("readtrailer error:%d",1);
	...
	{
		fz_throw("first object in xref is not free");
		xref->table[0].type = 'f';
	}
	...
	{
		fz_throw("object (%d %d R) has invalid offset, assumed missing", i, xref->table[i].gen);
		xref->table[i].type = 'f';
	}
