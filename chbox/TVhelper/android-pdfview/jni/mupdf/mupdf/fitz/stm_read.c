...
fz_readimp(fz_stream *stm)
{
	...
	case FZ_SFILE:
		...
		else if(stm->fsocket.handle != nil)
		{
			int count = buf->ep - buf->wp;
			if(stm->fsocket.fsize != 0)
			{
				if(stm->fsocket.fsize < stm->fsocket.pos + count)
				{
					count = stm->fsocket.fsize - stm->fsocket.pos;
				}
			}
			n = HTTP_GetContent(stm,buf->wp,count);
			if(n == -1)
			{
				stm->dead = 1;
				return fz_throw("syserr: read: %s", strerror(errno));
...
fz_tell(fz_stream *stm)
{
	...
	case FZ_SFILE:
		...
		else if(stm->fsocket.handle != NULL)
		{			
			return stm->fsocket.pos - (buf->wp - buf->rp);
		}
		...
		else if(stm->fsocket.handle != NULL)
		{
			switch(whence)
			{
				...
				case 1:
				{
					...
					if(stm->fsocket.pos + offset >= 0
						&& stm->fsocket.pos + offset <= stm->fsocket.fsize)
					{
						stm->fsocket.pos += offset;
					}
					else
					{
						stm->dead = 1;
				...
				{
					if(offset <= 0
						&& stm->fsocket.fsize + offset <= stm->fsocket.fsize)
					{
						stm->fsocket.pos = stm->fsocket.fsize + offset;
					}
					else
					{
						stm->dead = 1;
			...
			fz_throw("seek file:%d,fsize:%d",stm->fsocket.pos,stm->fsocket.fsize);
			buf->rp = buf->bp;
			buf->wp = buf->bp;
			return fz_okay;
		}
