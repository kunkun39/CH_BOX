...
public class ReaderView extends AdapterView<Adapter> implements
	...
	public void moveToNextTop() {
		View v = mChildViews.get(mCurrent + 1);
		if (v != null)
			slideViewOntoTopScreen(v);
	}
	...
	public  void scrollDistance(int dy)
	{
		...
		if ( dy < 0) // 当往下滚动时
		{
			if (( -v.getTop() + getHeight()) % v.getMeasuredHeight() == 0) // 当滚动到临界的地方时
			{
				moveToNext(); //滚动到下一页
			}
			else
			{
				mScrollerLastY = 0;
				// 计算到底部还有多少距离，然后滚动
				int sdy = smartAdvanceAmount(getHeight(),v.getMeasuredHeight()-((-v.getTop() + getHeight()) % v.getMeasuredHeight()));
				if (sdy >= -dy)
				{
					sdy = dy;
				}
				...
				mScroller.startScroll(corr.x, corr.y + v.getTop(), 0, sdy,100);
				mStepper.prod();
			}
		}else if (dy > 0) // 当往上滚动时
		{
			if (((-v.getTop()) % v.getMeasuredHeight() == 0)) // 当滚动到临界的地方时
			{
				moveToPrevious();// 滚动到上一页
			}
			else
			{
				mScrollerLastY = 0;
				// 计算到顶部还有多少距离，然后滚动
				int sdy = smartAdvanceAmount(getHeight(),((-v.getTop()) % v.getMeasuredHeight()));
				if (sdy >= dy)
				{
					sdy = dy;
				}

				mScroller.startScroll(corr.x,corr.y, 0, sdy,100);
				mStepper.prod();
	...
	public void onKeyDrag(float distanceX, float distanceY) {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		try {
			...
			if (!memAlert) {
				memAlert = true;
				AlertDialog alertDialog = PDFViewActivity.getAlertBuilder()
						.create();
	...
	private void onLayout2(boolean changed, int left, int top, int right,
		...
		if (mCurrent > 0) {
			...
			if (HORIZONTAL_SCROLLING) {
				...
				lv.layout(cvLeft - lv.getMeasuredWidth() - gap, (cvBottom
						+ cvTop - lv.getMeasuredHeight()) / 2, cvLeft - gap,
						(cvBottom + cvTop + lv.getMeasuredHeight()) / 2);
			} else {
				int gap = leftOffset.y + GAP + cvOffset.y;
				lv.layout((cvLeft + cvRight - lv.getMeasuredWidth()) / 2, cvTop
						- lv.getMeasuredHeight() - gap,
						(cvLeft + cvRight + lv.getMeasuredWidth()) / 2, cvTop
								- gap);
			}
		}
		...
		if (mCurrent + 1 < mAdapter.getCount()) {
			...
			if (HORIZONTAL_SCROLLING) {
				...
				rv.layout(cvRight + gap,
						...
						cvRight + rv.getMeasuredWidth() + gap, (cvBottom
	...
	private void slideViewOntoTopScreen(View v) {
		Point corr = getCorrection(getScrollBounds(v));
